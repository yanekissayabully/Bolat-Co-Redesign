...
export function LeadForm() {
  ...
  // Отправка события в Facebook Pixel (с дополнительными данными)
  const sendFacebookLeadEvent = () => {
    if (typeof window !== 'undefined' && window.fbq && facebookData) {
      ...
      console.log('✅ Facebook Pixel Lead event sent with identifiers')
    } else {
  ...
  const sendToTelegram = async () => {
    // Формируем данные для отправки, включая Facebook поля
    const fullData = {
      ...answers,
